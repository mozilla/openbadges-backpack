...
function revokeOrigin(req, res, next) {
  if (!req.user)
    return res.send(403);
  if (!req.body)
    return res.type('text').status(400).send('body expected');
  if (!req.body.origin)
    return res.type('text').status(400).send('origin URL expected');
...
function refresh(req, res, next) {
  ...
  if (req.body.grant_type != "refresh_token")
    return res.type('text').status(400).send('invalid grant_type');
  ...
  this.Model.find({refresh_token: refresh_token}, function(err, results) {
    ...
    if (!session)
      return res.type('text').status(400).send('invalid refresh_token');
    if (req.headers['origin']) {
      res.set('access-control-allow-origin', session.get('origin'));
      if (session.get('origin') != req.headers['origin'])
        return res.type('text').status(401).send("invalid origin");
    }
...
function requestAccess(req, res) {
  if (!req.query.callback)
    return res.type('text').status(400).send('callback expected');
  if (!req.query.scope)
    return res.type('text').status(400).send('scope expected');
  ...
  if (originErr)
    return res.type('text').status(400).send('invalid callback: ' + originErr);
  if (scopeErr)
    return res.type('text').status(400).send('invalid scope: ' + scopeErr);
...
function authorize(permission, req, res, next) {
  ...
  var tokenError = function(type, desc) {
    ...
    return res.type('text').status(401).send(type + ": " + desc);
  };
  var invalidTokenError = tokenError.bind(null, "invalid_token");

  if (!auth) {
    res.header('WWW-Authenticate', bearerRealmStr);
    return res.type('text').status(401).send("access token expected");
  }
