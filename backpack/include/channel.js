            ...
            var onMessage = function(origin, method, m) {
                ...
                if (m.id && method) {
                    ...
                    if (!outTbl[m.id]) {
                        ...
                        // XXX: what if client code raises an exception here?
                        if (m.error) {
                            var errorCallback = (1,outTbl[m.id].error);
                            if (typeof errorCallback !== "function") {
                                throw "Error: Call to method '" + m.message + "' failed and no error handler was provided.";
                            }
                            errorCallback(m.error, m.message);
                        } else {
...
Channel.build({
  ...
  origin: "*",
  scope: "badges"
}).bind("add", function(event, data) {
  console.dir(data);
  return "adding";
}).bind("update", function(event, data) {
  return "updating";
}).bind("revoke", function(event, data) {
  return "revoking";
}).bind("setKey", function(event, data) {
  return "setting key";
});