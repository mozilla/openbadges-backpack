function inlineL10nTests(InlineL10n, test, equal, deepEqual) {
...
if (typeof(test) != 'undefined')
  // QUnit in browser
  inlineL10nTests(InlineL10n, test, equal, deepEqual);
else (function() {
  // TAP in Node
  var currentTest;

  inlineL10nTests(require('../lib/inline-l10n'), function test(name, fn) {
    require('tap').test(name, function(t) {
      currentTest = t;
      fn();
      t.end();
    });
  }, function equal() {
    currentTest.equal.apply(currentTest, arguments);
  }, function deepEqual() {
    currentTest.deepEqual.apply(currentTest, arguments);
  });
})();
