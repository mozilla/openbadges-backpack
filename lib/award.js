...
function award(opts, callback) {
  ...
  var originalImage = opts.imagedata||opts.imageData;

  // if we're dealing with an svg, let's strip any scripts before we do anything else
  if (extension == '.svg') {
    var svgData = originalImage.toString('utf8');

    var $ = cheerio.load(svgData, {xmlMode: true});

    $('script').each(function() {
      $(this).remove();
    });

    originalImage = Buffer($.xml());
  }
  ...
  async.parallel({
    ...
    const badge = new Badge({
      ...
      awarded_to_email_address: opts.awardedTo,
      endpoint: opts.url,
