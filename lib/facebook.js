...
function publishOpenGraphObject (opts, callback) {
  request({
	  method: 'POST',
    uri: 'https://graph.facebook.com/' + opts.userId + '/' + opts.fbNamespace + ':' + opts.action,
    qs: {
...
exports.publishBadge = function (accessToken, badgeBodyHash, userId, callback) {
  ...
  publishOpenGraphObject({'userId': userId, 'accessToken': accessToken, 'fbNamespace': fbNamespace, 'action': action, 'objectUrl': objectUrl}, function(error, response) {
...
exports.publishComment = function (objectId, accessToken, comment, callback) {
	request({
		method: 'POST',
		uri: 'https://graph.facebook.com/' + objectId + '/comments',
    qs: {
